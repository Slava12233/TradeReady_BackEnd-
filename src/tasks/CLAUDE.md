...
| File | Purpose |
|------|---------|
| `celery_app.py` | Celery app factory, broker/backend config, queue definitions, full beat schedule. The `agent.tasks` module is conditionally included via `importlib.util.find_spec()` (not direct import) to avoid circular ImportError — `agent/tasks.py` imports `app` from this module. |
...
- **`agent.tasks` import is optional**: `celery_app.py` uses `importlib.util.find_spec("agent.tasks")` to check availability without importing. Direct import would cause a circular ImportError because `agent/tasks.py` imports `app` from this module. If the package is present (e.g., in the agent Docker profile), the module name is added to the `include` list and Celery auto-discovers the tasks.
