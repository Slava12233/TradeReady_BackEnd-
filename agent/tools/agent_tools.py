...
scan_opportunities, journal_entry, and agent.task.request_platform_feature.
...
# Maximum number of opportunities returned by agent.task.scan_opportunities.
...
def get_agent_tools(config: AgentConfig, agent_id: str) -> list[Any]:  # noqa: ANN401
    ...
    async def reflect_on_trade(  # noqa: ANN202
        ...
        """
        async with log_api_call("db", "reflect_on_trade", trade_id=trade_id) as _log_ctx:
            try:
                ...
                if trade_data is None:
                    logger.warning(
                        "agent.task.reflect_on_trade.trade_not_found",
                        trade_id=trade_id,
                        agent_id=agent_id,
                    )
                ...
                try:
                    ...
                    logger.warning(
                        "agent.task.reflect_on_trade.price_fetch_failed",
                        symbol=symbol,
                        error=str(exc),
                    )
                ...
                try:
                    ...
                    logger.warning(
                        "agent.task.reflect_on_trade.candles_fetch_failed",
                        symbol=symbol,
                        error=str(exc),
                    )
                ...
                try:
                    ...
                    try:
                        ...
                        logger.warning(
                            "agent.task.reflect_on_trade.observations_fetch_failed",
                            agent_id=agent_id,
                            error=str(db_exc),
                        )
                        await session.close()
                    ...
                    logger.warning(
                        "agent.task.reflect_on_trade.observations_unavailable",
                        agent_id=agent_id,
                        error=str(exc),
                    )
                ...
                try:
                    ...
                    try:
                        ...
                        journal_entry_row = AgentJournal(
                            agent_id=UUID(agent_id),
                        ...
                        await journal_repo.create(journal_entry_row)
                        ...
                        logger.info(
                            "agent.task.reflect_on_trade.persisted",
                            trade_id=trade_id,
                            agent_id=agent_id,
                        ...
                        logger.warning(
                            "agent.task.reflect_on_trade.persist_failed",
                            trade_id=trade_id,
                            agent_id=agent_id,
                            error=str(db_exc),
                        )
                    ...
                    logger.warning(
                        "agent.task.reflect_on_trade.db_unavailable",
                        trade_id=trade_id,
                        agent_id=agent_id,
                        error=str(exc),
                    )

                _log_ctx["response_status"] = 200
                return reflection.model_dump(mode="json")

            except AgentExchangeError as exc:
                logger.warning(
                    "agent.task.reflect_on_trade.sdk_error",
                    trade_id=trade_id,
                    agent_id=agent_id,
                    error=str(exc),
                )
                ...
                logger.exception(
                    "agent.task.reflect_on_trade.unexpected_error",
                    trade_id=trade_id,
                    agent_id=agent_id,
                    error=str(exc),
                )
    ...
    async def review_portfolio(  # noqa: ANN202
        ...
        Returns:
            ...
            On failure returns ``{"error": "<message>"}``.
        """
        async with log_api_call("db", "review_portfolio") as _log_ctx:
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        logger.info(
                            "agent.task.review_portfolio.no_budget_record",
                            agent_id=agent_id,
                        )
                    except DBError as db_exc:
                        logger.warning(
                            "agent.task.review_portfolio.budget_fetch_failed",
                            agent_id=agent_id,
                            error=str(db_exc),
                        )
                    finally:
                    ...
                    logger.warning(
                        "agent.task.review_portfolio.budget_unavailable",
                        agent_id=agent_id,
                        error=str(exc),
                    )
                ...
                try:
                    ...
                    try:
                        ...
                        logger.info(
                            "agent.task.review_portfolio.persisted",
                            agent_id=agent_id,
                            health_score=health_score,
                        ...
                        logger.warning(
                            "agent.task.review_portfolio.persist_failed",
                            agent_id=agent_id,
                            error=str(db_exc),
                        )
                    finally:
                    ...
                    logger.warning(
                        "agent.task.review_portfolio.db_unavailable",
                        agent_id=agent_id,
                        error=str(exc),
                    )
                ...
                logger.warning(
                    "agent.task.review_portfolio.sdk_error",
                    agent_id=agent_id,
                    error=str(exc),
                )
                return {"error": str(exc)}
                ...
                logger.exception(
                    "agent.task.review_portfolio.unexpected_error",
                    agent_id=agent_id,
                    error=str(exc),
                )
    ...
    async def scan_opportunities(  # noqa: ANN202
        ...
        """
        async with log_api_call("db", "scan_opportunities") as _log_ctx:
            try:
                ...
                try:
                    ...
                    logger.info(
                        "agent.task.scan_opportunities.prices_fetched",
                        count=len(prices_raw),
                        agent_id=agent_id,
                    )
                except Exception as exc:  # noqa: BLE001
                    logger.warning(
                        "agent.task.scan_opportunities.redis_unavailable",
                        agent_id=agent_id,
                        error=str(exc),
                    )
                ...
                try:
                    ...
                    logger.warning(
                        "agent.task.scan_opportunities.positions_fetch_failed",
                        agent_id=agent_id,
                        error=str(exc),
                    )
                ...
                try:
                    ...
                    logger.warning(
                        "agent.task.scan_opportunities.ticker_fetch_failed",
                        error=str(exc),
                    )
                ...
                logger.info(
                    "agent.task.scan_opportunities.complete",
                    agent_id=agent_id,
                ...
                logger.exception(
                    "agent.task.scan_opportunities.unexpected_error",
                    agent_id=agent_id,
                    error=str(exc),
                )
    ...
    async def journal_entry(  # noqa: ANN202
        ...
        async with log_api_call("db", "journal_entry", entry_type=entry_type) as _log_ctx:
            try:
                ...
                try:
                    ...
                    logger.warning(
                        "agent.task.journal_entry.price_fetch_failed",
                        agent_id=agent_id,
                        error=str(exc),
                    )
                ...
                try:
                    ...
                    logger.warning(
                        "agent.task.journal_entry.positions_fetch_failed",
                        agent_id=agent_id,
                        error=str(exc),
                    )
                ...
                try:
                    ...
                    try:
                        ...
                        title = (
                            first_line
                            if first_line
                            else f"Journal entry — {_now_utc().strftime('%Y-%m-%d %H:%M UTC')}"
                        )
                        ...
                        logger.info(
                            "agent.task.journal_entry.persisted",
                            entry_id=saved_id,
                            agent_id=agent_id,
                            entry_type=db_entry_type,
                            tags=auto_tags,
                        ...
                        await db_session.rollback()
                        logger.warning(
                            "agent.task.journal_entry.persist_failed",
                            agent_id=agent_id,
                            error=str(db_exc),
                        )
                    finally:
                    ...
                    logger.warning(
                        "agent.task.journal_entry.db_unavailable",
                        agent_id=agent_id,
                        error=str(exc),
                    )
                ...
                logger.exception(
                    "agent.task.journal_entry.unexpected_error",
                    agent_id=agent_id,
                    error=str(exc),
                )
    ...
    async def request_platform_feature(  # noqa: ANN202
        ...
        async with log_api_call("db", "request_platform_feature", category=category) as _log_ctx:
            try:
                ...
                try:
                    ...
                    if existing is not None:
                        ...
                        logger.info(
                            "agent.task.request_platform_feature.duplicate_detected",
                    ...
                    logger.info(
                        "agent.task.request_platform_feature.persisted",
                    ...
                    await db_session.rollback()
                    logger.warning(
                        "agent.task.request_platform_feature.persist_failed",
                        agent_id=agent_id,
                        error=str(db_exc),
                    )
                ...
                logger.exception(
                    "agent.task.request_platform_feature.unexpected_error",
                    agent_id=agent_id,
                    error=str(exc),
                )
