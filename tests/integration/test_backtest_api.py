...
async def test_create_backtest_requires_auth(client: AsyncClient) -> None:
    ...
    resp = await client.post("/api/v1/backtest/create", json={
...
async def test_create_backtest_invalid_balance(
...
async def test_step_nonexistent_session(
    client: AsyncClient, auth_headers: dict[str, str]
) -> None:
...
async def test_data_range_endpoint(
    client: AsyncClient, auth_headers: dict[str, str]
) -> None:
...
async def test_list_backtests_empty(
    client: AsyncClient, auth_headers: dict[str, str]
) -> None:
...
async def test_best_backtest_not_found(
    client: AsyncClient, auth_headers: dict[str, str]
) -> None:
    """Best backtest with no completed tests should return 404."""
    resp = await client.get(
        "/api/v1/backtest/best?metric=roi_pct", headers=auth_headers
    )
    assert resp.status_code == 404


async def test_account_mode_default(
    client: AsyncClient, auth_headers: dict[str, str]
) -> None:
...
async def test_switch_mode_invalid(
    ...
    resp = await client.post("/api/v1/account/mode", headers=auth_headers, json={
        "mode": "invalid",
    })
    assert resp.status_code == 422
